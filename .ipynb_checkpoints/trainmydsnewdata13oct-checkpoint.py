...
NEW_SAVE_PATH = "D:/Deep_Learning/Grounding-Dino-FineTuning/weights_myds_NEWLY_TRAINED"  # new folder for saving

train_images_dir = r"D:\Deep_Learning\Dataset\Phone_crack_data_partial\images\train"
train_labels_dir = r"D:\Deep_Learning\Dataset\Phone_crack_data_partial\labels\train"
...
def train(model, train_img_dir, train_lbl_dir,
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if (epoch+1) % save_epoch == 0 or (epoch+1) == 1:
            save_path = os.path.join(NEW_SAVE_PATH, f"epoch_{epoch+1}.pth")
            print(f"💾 Saving checkpoint: {save_path}")
            torch.save(model.state_dict(), save_path)

    # Final save
    final_path = os.path.join(NEW_SAVE_PATH, "final_model.pth")
