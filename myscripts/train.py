...
def train(model, train_ann_file, val_ann_file=None, epochs=1000, 
          save_path='./weights_myds', save_epoch=2, early_stop_patience=15):
    """Training with complete logging and weight saving"""
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_ann_Dict.items(), desc=f"Train Epoch {epoch+1}") as pbar:
            for IMAGE_PATH, vals in pbar:
                try:
                    ...
                    #print(f"Image tensor is on: {'GPU' if image.is_cuda else 'CPU'}", end='\r')  
        ...
        if val_ann_Dict:
            ...
            with tqdm(val_ann_Dict.items(), desc=f"Val Epoch {epoch+1}") as pbar:
                with torch.no_grad():
                    for val_path, vals in pbar:
                        try:
                            ...
                            print(f"\nSkipped validation {val_path}: {str(e)}")
                            continue
        ...
        # Save weights periodically and on best validation
        if (epoch+1) % save_epoch == 0 or (epoch+1) == 1:  # Always save first epoch
